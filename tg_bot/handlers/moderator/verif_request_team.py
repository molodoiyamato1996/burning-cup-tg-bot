...
async def view_request_team(call: types.CallbackQuery, state=FSMContext):
    ...
    print(request_team.request_status)
...
async def verif_request_team(call: types.CallbackQuery, state=FSMContext):
    ...
    if result == 'yes':
        ...
        team_players = await db_model.get_team_players_without_captain(team_id=request_team.team_id, captain_id=captain.id)
        ...
        if len(tournament_teams) == registration.count_teams:
            ...
            for user in users:
                await notify_user(
                    text='Регистрация на турнир закончена <b>🔥 Burning Cup</b>\n\n'
                         'Команды и игры можете посмотреть на сайте https://www.poshel_nahui.com',
                    chat_id=user.id,
                    call=call
                )
